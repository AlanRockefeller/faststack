...
def test_no_nan_inf():
    ...
    test_cases = [
        ...
        np.array([[[0, 0, 0], [1e-10, 1e-10, 1e-10], [10.0, 5.0, 2.0]]], dtype=np.float32),  # Edge cases
    ]
...
def test_analyze_highlight_state():
    ...
    assert state['headroom_pct'] > 0.9, f"Should detect headroom: {state['headroom_pct']}"
    
    # Normal image
    normal_img = np.ones((10, 10, 3), dtype=np.float32) * 0.5
    state = _analyze_highlight_state(normal_img)
    assert state['headroom_pct'] < 0.01, f"Should not detect headroom: {state['headroom_pct']}"
