...
class TestEditorRotation(unittest.TestCase):
    ...
    def test_rotate_cw(self):
        ...
        # New TR (was TL Red)
        np.testing.assert_allclose(res[qh, w-qw], [1, 0, 0], err_msg="TR should be Red")
        # New BL (was BR White)
        np.testing.assert_allclose(res[h-qh, qw], [1, 1, 1], err_msg="BL should be White")
        # New BR (was TR Green)
        np.testing.assert_allclose(res[h-qh, w-qw], [0, 1, 0], err_msg="BR should be Green")
