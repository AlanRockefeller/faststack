        ...
        with patch('faststack.imaging.editor.create_backup_file', return_value=Path("backup.jpg")):
             ...
             with patch('PIL.Image.fromarray', return_value=mock_img):
                 try:
                     ...
                     print(f"FAILURE: save_image raised unexpected exception type: {type(e)} {e}")
                     return False
    except Exception as e:
        print(f"Unexpected error in test setup: {e}")
        return False
...
if __name__ == '__main__':
    ...
    success = True
    if not test_load_image_raises(): success = False
    print("-" * 20)
    if not test_save_image_raises(): success = False
