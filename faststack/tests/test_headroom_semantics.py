...
from faststack.imaging.editor import _analyze_highlight_state
...
class TestHeadroomSemantics(unittest.TestCase):
    def test_headroom_exposure_independence(self):
        ...
        self.assertEqual(
            state_exposed["headroom_pct"],
            0.0,
            "Headroom should not be triggering just because of exposure",
        )
        ...
        self.assertGreater(
            state_naive["headroom_pct"],
            0.99,
            "Naive analysis should show headroom (sanity check)",
        )
