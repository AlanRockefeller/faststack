...
def _analyze_highlight_state(rgb_linear: np.ndarray, srgb_u8: Optional[np.ndarray] = None, pre_exposure_linear: Optional[np.ndarray] = None) -> dict:
    ...
    if srgb_u8 is not None and srgb_u8.shape[:2] == rgb_linear.shape[:2]:
        ...
        source_clipped_pct = float(np.count_nonzero(max_to_check >= _LINEAR_THRESHOLD_254)) / total_pixels
...
def _highlight_recover_linear(
    ...
    *,
    ...
    if chroma_rolloff > 0.001:
        ...
        extreme_mask = _smoothstep01((target_brightness - _CHROMA_ROLLOFF_START * headroom_ceiling) / (_CHROMA_ROLLOFF_WIDTH * headroom_ceiling))
        extreme_mask = np.expand_dims(extreme_mask, axis=2)
        
        # Compute grayscale (luminance) of recovered image
        gray = recovered[:, :, 0:1] * 0.2126 + recovered[:, :, 1:2] * 0.7152 + recovered[:, :, 2:3] * 0.0722
